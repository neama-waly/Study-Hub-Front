...
function Questions(){
    ...
    if (!token) {
        ...
        const LoadQuestions = useCallback(async (query = "") =>{
            try{
                const token = localStorage.getItem("token");
                setLoading(true);
                const data = await fetch(`https://study-hub-brown-mu.vercel.app/api/questions?search=${encodeURIComponent(query)}`,{
                    method: "GET",
                    headers: {
        ...
        const addQuestion = async(e)=>{
            ...
            try{
                const respone = await fetch('https://study-hub-brown-mu.vercel.app/api/questions',{
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": `Bearer ${token}`
    ...
    const deleteOne = async(id)=>{
            try{
                const response = await fetch(`https://study-hub-brown-mu.vercel.app/api/questions/${id}`, {
            method: 'delete', 
        });  
        if (response.ok){
