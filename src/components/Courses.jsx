...
function Courses(){
    ...
    const LoadCourses =useCallback(async (query = "") =>{
        try{
            const token = localStorage.getItem("token");
            setLoading(true);
            const response = await fetch(`https://study-hub-brown-mu.vercel.app/api/materials?search=${encodeURIComponent(query)}`,{
                    method: "GET",
                    headers: {
    ...
    const addCourse = async(e)=>{
        ...
        try{
            const response = await fetch("https://study-hub-brown-mu.vercel.app/api/materials",{
                method : "POST",
                headers : {
                    "Authorization": `Bearer ${token}`
                },
    ...
    const deleteCourse = async(id)=>{
        const token = localStorage.getItem("token");
        try{
            const response = await fetch(`https://study-hub-brown-mu.vercel.app/api/materials/${id}`,{
                method : "DELETE",
                headers : {
                    "Authorization": `Bearer ${token}`
                }
    ...
    const UpdateProgress = async(courseId , newProgress)=>{
        ...
        try{
            const token = localStorage.getItem("token");
            const response =await fetch(`https://study-hub-brown-mu.vercel.app/api/materials/${courseId}/progress`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
